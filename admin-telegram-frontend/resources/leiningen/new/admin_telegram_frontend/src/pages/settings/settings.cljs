(ns {{name}}.pages.settings.settings
  (:require
   ["antd" :as antd]
   ["@ant-design/icons" :as icons]
   [{{name}}.db :refer [app-state]]
   [{{name}}.api-uri-maker :refer [api_uri_maker banner_uri_maker]]
   [{{name}}.pages.settings.modal-banner-edit :refer [modal_banner_edit]]
   [{{name}}.pages.settings.banner-list :refer [banner_list]]
   [reagent.core :as reagent :refer [as-element]]
   [{{name}}.events.settings-set :refer [settings_set]]
   [{{name}}.events.catalog-bot-description-set :refer [catalog_bot_description_set]]
   [{{name}}.pages.settings.payment-methods :refer [payment_methods]]
   [{{name}}.pages.settings.delivery-methods :refer [delivery_methods]]
   [{{name}}.pages.settings.managers :refer [managers]]
   [{{name}}.pages.settings.modal-help :refer [modal_help]]
   )
)
