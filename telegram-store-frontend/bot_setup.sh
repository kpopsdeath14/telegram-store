...
MENU_BUTTON_DATA="{\"menu_button\":{\"type\":\"web_app\",\"text\":\"📚 Открыть каталог\",\"web_app\":{\"url\":\"$WEBAPP_URL\"}}}"

result=$(make_request "POST" "/setChatMenuButton" "$MENU_BUTTON_DATA")
if [ $? -eq 0 ]; then
    echo "✓ Веб-приложение активировано"
    echo "  URL: $WEBAPP_URL"
    echo "  Кнопка меню: '📚 Открыть каталог'"
else
...
# 3. Настройка URL для веб-приложения (через BotFather)
...
WEBHOOK_DATA="{\"url\":\"$WEBHOOK_URL\",\"max_connections\":100,\"allowed_updates\":[\"message\",\"callback_query\",\"inline_query\",\"chat_member\"]}"
...
if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        # Показываем текущие команды бота
        echo "   Проверяю установленные команды..."
        commands_result=$(make_request "GET" "/getMyCommands" "{\"scope\":{\"type\":\"default\"}}")
        if [ $? -eq 0 ]; then
            echo "   Команды подтверждены"
        fi
    fi
else
